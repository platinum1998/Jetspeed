...
export class BlockGenerator {
    ...
    public static BlockWithinRangeZ(player_pos_z: number, last_block_back: number, threshold: number): boolean {
        return (Math.floor(Math.abs(last_block_back - player_pos_z)) <= threshold)
    }

    // Initialise block patch (for start of game)
    public static Initialise(scene: BABYLON.Scene, assetsManager: BABYLON.AssetsManager, block_list: Block[]): void {
        
        const meshTask = assetsManager.addMeshTask('piano task', '', 'assets/geometry/', 'block.babylon');
    ...
    public static GenerateBlocksRealtime(
        ...
        block_list: Block[]) {
            ...
            if (BlockGenerator.BlockWithinRangeZ(player_pos_z, last_block_back, fog_start)) {
                for (let rows = 0; rows < num_rows; rows++) {   // Iterate through each row of blocks 
